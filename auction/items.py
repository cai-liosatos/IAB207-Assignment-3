...
def create():
    if request.method == 'POST':
        ...
        item=Item(name=name, category=category, manufacturer=manufacturer, condition=condition, image=image, finishDate=finishdate, deliveryTime=postagedate, currentPrice=startingprice, postagePrice=postageprice, currency=currency, moreInfo=description)
        db.session.add(item)
        db.session.commit()
